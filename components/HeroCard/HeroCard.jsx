...
export default function HeroCard({hero}) {
    ...
    const handlePress = async(hero) => {
        if(isFav){
          ...
          await favoriteHeroesAsyncStorage.addHero({name: hero.name, gender: hero.gender});
          const newHeroes = await favoriteHeroesAsyncStorage.getHeroes()
          setFavHeroes(newHeroes)
        }
        setIsFav((prevState) => !prevState);  // Toggle state after action
    }
    ...
    useEffect(() => {
      (async () => {
        try {
          ...
          const heroExists = favHeroesFromStore.some((favHero) => favHero.name === hero.name);
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.learnMoreContainer}>
        <Text style={styles.learnMoreTxt} onPress={() => handleNavigate()}>Learn More</Text>
      </TouchableOpacity>
    </View>
  );
}
